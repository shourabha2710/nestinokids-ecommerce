...
import { adminHeroAPI } from '../../api/endpoints';
import {
...
const HeroSlides = () => {
  ...
  const fetchSlides = useCallback(async () => {
    try {
      setLoading(true);
      const res = await adminHeroAPI.getSlides();
      setSlides(res.data);
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await adminHeroAPI.updateSlide(editingId, fd);
        showToast('Hero slide updated successfully');
      } else {
        await adminHeroAPI.createSlide(fd);
  ...
  const toggleActive = async (id, isActive) => {
    ...
    try {
      ...
      await adminHeroAPI.updateSlide(id, fd);
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      await adminHeroAPI.reorder(updated.map((s) => ({ id: s.id, display_order: s.display_order })));
    } catch {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await adminHeroAPI.deleteSlide(deleteId);
      setDeleteId(null);
