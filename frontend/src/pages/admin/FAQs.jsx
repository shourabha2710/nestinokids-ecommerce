...
import { adminFAQAPI } from '../../api/endpoints';
import { HelpCircle, Plus, X, Search, ChevronUp, ChevronDown } from 'lucide-react';
...
const FAQs = () => {
  ...
  const handleSave = async () => {
    try {
      if (editItem) {
        await adminFAQAPI.updateFAQ(editItem.id, form);
      } else {
        await adminFAQAPI.createFAQ(form);
      }
      setShowModal(false);
      fetchFAQs();
    } catch {}
  ...
  const toggleActive = async (faq) => {
    try {
      await adminFAQAPI.updateFAQ(faq.id, { is_active: !faq.is_active });
      fetchFAQs();
    } catch {}
  };
          ...
          {categories.map((cat) => (
            <div key={cat} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {grouped[cat].map((faq) => (
                  <div key={faq.id} className="px-5 py-3.5 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-400 mt-1">Order: {faq.display_order}</p>
                      </div>
                      <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        <button onClick={() => openEdit(faq)} className="p-1.5 hover:bg-gray-100 rounded-lg text-xs text-gray-500">
                          Edit
                        </button>
