...
const Announcements = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editItem) {
        await adminAPI.updateAnnouncement(editItem.id, data);
      } else {
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-text-muted mt-1">Manage announcement bar messages</p>
        </div>
        <button
          onClick={openCreate}
          ...
          {announcements.map((item) => (
            <div
              ...
              className={`bg-white rounded-xl shadow-sm border p-5 transition-all ${
                isExpired(item) ? 'border-border opacity-60' : 'border-border hover:shadow-md'
              }`}
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="font-medium text-text text-sm">{item.message}</p>
                  </div>
                  {item.link && (
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => openEdit(item)} className="p-1.5 hover:bg-gray-100 rounded-lg text-xs text-text-muted">
                    Edit
                  </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-text mb-1">Link (optional)</label>
                <input
                  type="text"
                  value={form.link}
