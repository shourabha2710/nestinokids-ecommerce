...
const WebsiteSettings = () => {
  ...
  const inputClass = 'w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm text-text outline-none focus:border-gold/50 focus:ring-2 focus:ring-gold/30 transition-all';
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-text">Website Settings</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {businessFields.map((field) => (
              <div key={field.key}>
                ...
                <div className="relative">
                  <field.icon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted" />
                  {field.textarea ? (
                    <textarea
        ...
        <div className="flex justify-end">
          <motion.button
            ...
            disabled={saving}
            whileTap={{ scale: 0.98 }}
            className="h-11 px-8 bg-gold text-white rounded-xl font-semibold text-sm hover:bg-gold-dark transition-colors flex items-center gap-2 disabled:opacity-50"
          >
