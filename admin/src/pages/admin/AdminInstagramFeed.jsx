...
const AdminInstagramFeed = () => {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      setLoading(true);
      const res = await adminAPI.getInstagramPosts();
      setPosts(res.data);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await adminAPI.updateInstagramPost(editingId, formData);
        showToast('Instagram post updated successfully');
      } else {
        await adminAPI.createInstagramPost(formData);
  ...
  const toggleActive = async (id, isActive) => {
    ...
    try {
      await adminAPI.toggleInstagramPost(id, isActive);
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      await adminAPI.reorderInstagramPosts(
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await adminAPI.deleteInstagramPost(deleteId);
      setDeleteId(null);
